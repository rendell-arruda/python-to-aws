...
def lambda_handler(event, context):
    try:
        ...
        for region in regions:
            ...
            for volume in ec2.volumes.all():
                ...
                if not volume.attachments:
                    ...
                    if (
                        "inUse" in volume_info["Tags"]
                        and volume_info["Tags"]["inUse"] == "true"
                    ):
                        print(
                            f"Volume {volume.id} não pode ser excluído pois está marcado como emUso"
                        )
                        volumes_com_erro += 1
                    else:
                        # volume.delete()
        ...
        send_email(
            "Resumo da Exclusão de Volumes EBS",
            f"Volumes excluídos com sucesso: {volumes_excluidos}\nVolumes com erro: {volumes_com_erro}",
        )

    except Exception as e:
        print(f"Ocorreu um erro: {e}")
        send_email(
            "Erro durante a execução da Lambda",
            f"Ocorreu um erro durante a execução da Lambda: {e}",
        )
